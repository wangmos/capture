...
**UIA never runs on the UI thread here.** `AutomationElement.FromPoint` is cross-process COM and blocks for seconds against a hung target — enough to freeze the whole overlay, Esc included. `TakeUiaRefinement` therefore returns the *previous* background probe's result (only if it was taken within 2 px of the current point) and queues a new probe, one in flight at a time. A probe that never returns degrades to Win32-only detection instead of hanging the UI. The refinement lags one detection round (~40 ms), which the 60 ms debounce already absorbs.

**Overlay render path.** The overlay window keeps `AllowsTransparency="False"` (see the comment in `OverlayWindow.xaml`) — enabling it switches WPF to the layered-window/software path, and this is a full-screen surface repainted on every mouse move. The dim mask is drawn as four edge rectangles rather than `CombinedGeometry(Exclude)` for the same reason, and `Annotation.BrushOf`/`PenOf` cache their frozen brushes and pens because every annotation re-renders each frame during a drag.

**Keyboard shortcuts are user-rebindable.** `Core/ShortcutMap.cs` maps a `ShortcutAction` (17 of them — 8 tools plus undo/select-all/OCR/long-shot/pin/save/copy/confirm/exit) to a `ShortcutDef` (key + Ctrl/Alt/Shift, serialized as `"Ctrl+S"` / `"R"` in `AppSettings.Shortcuts`). `SessionModel.OnKey` resolves the pressed key through the map and dispatches via `Execute`; assigning a combination that is already taken clears the previous owner. Defaults: bare letters for tools (R/O/A/P/T/M/N, I for 取字), Ctrl combos for actions. Two deliberate carve-outs: Esc always falls through to the exit logic even if rebound (never trap the user in the overlay), and Esc cannot be *assigned* from the settings UI because it means "cancel recording" there. Toolbar tooltips are rewritten from the live map by `ToolbarControl.ApplyShortcuts`, so they never drift from the real binding.
